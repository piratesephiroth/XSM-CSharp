...
public class State : Node
{
	...
	public void InitChildrenStateMap(Godot.Collections.Dictionary<string, State> dict, StateRoot newStateRoot)
	{
							 ...
							 string argsOnExit = null)
	{
		...
		if (!IsRoot())
        {
			newStateNode.GetParent().EmitSignal(nameof(SubstateChanged), newStateNode);
        }
	...
	bool HasParent(State stateNode)
    {
		...
		if ( (GetParent().GetClass() != "State") || (parent == stateRoot) )
        {
			return false;
        }
	...
	{
		if ( !(status == INACTIVE) && !newActive)
		{
			status = INACTIVE;
			stateRoot.RemoveActiveState(this);
		}

		else if (newActive && !(status == ACTIVE) )
		{
			status = ACTIVE;
	...
	*/
	...
	void Stop(bool reset = true)
    {
		if ((status == ACTIVE) && (animPlayer != null))
        {
			...
			//stateRoot.currentAnimPriority = 0;
        }
	...
	// PRIVATE FUNCTIONS
	//
	public void InitChildrenStates(StateRoot rootState, bool firstBranch)
	{
		foreach (var c in GetChildren())
		{
			if (c.GetType() != typeof(State))
	...
	public void UpdateActiveStates(float _delta)
	{
		if (disabled)
		{
			return;
	...
	public void Enter(string args = null)
    {
		if (disabled)
        {
			return;
	...
	public void ResetDoneThisFrame(bool newDone)
    {
